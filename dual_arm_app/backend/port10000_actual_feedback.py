...
def extract_actual_joints_radians(packet: Mapping[str, Any]) -> list[float]:
    ...
    raw = packet.get("joint_actual_position")
    if (
        isinstance(raw, (str, bytes))
        or not isinstance(raw, Sequence)
        or len(raw) < JOINT_COUNT
    ):
        raise ValueError("joint_actual_position must contain at least six values")
    degrees: list[float] = []
    for index, value in enumerate(raw[:JOINT_COUNT]):
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            raise ValueError(f"joint_actual_position[{index}] must be finite")
        converted = float(value)
        if not math.isfinite(converted):
            raise ValueError(f"joint_actual_position[{index}] must be finite")
        degrees.append(converted)
